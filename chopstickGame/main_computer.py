...
def select_what_to_do():
    if player_turn == 1:
        ...
        print("[컴퓨터가 생각하는 중입니다...]\n")
        time.sleep(3)
        r = random.choice(list(["공격", "나누기"]))
        if r == "공격":
...
def get_first_player_input():
    hand = str(input("[선택할 당신의 왼손, 오른손 중에 하나를 입력해주세요]\n"))

    if hand == "왼손":
        atk_hand = str(input("[이제, 공격할 상대의 왼손, 오른손 중에 하나를 입력해주세요]\n"))
        if atk_hand == "왼손":
...
def get_second_player_input():
    h = ["left", "right"]
    hand = str(random.choice(h))

    if hand == "left":
        if player_second_cs_left == 0:
            get_second_player_input()
            return
        atk_hand = str(random.choice(h))
        if atk_hand == "left":
            add_cs("left", "left", "player2")
        elif atk_hand == "right":
            add_cs("left", "right", "player2")
    elif hand == "right":
        if player_second_cs_right == 0:
            get_second_player_input()
            return
        atk_hand = str(random.choice(h))
        if atk_hand == "left":
...
def add_cs(selected_hand, target_hand, now_turn):
    ...
    if selected_hand == "left" and target_hand == "left":
        if now_turn == "player1":
            player_second_cs_left += int(player_cs_left)
        elif now_turn == "player2":
            player_cs_left += int(player_second_cs_left)
    elif selected_hand == "left" and target_hand == "right":
        if now_turn == "player1":
            player_second_cs_right += int(player_cs_left)
        elif now_turn == "player2":
            player_cs_right += int(player_second_cs_left)
    elif selected_hand == "right" and target_hand == "left":
        if now_turn == "player1":
            player_second_cs_left += int(player_cs_right)
        elif now_turn == "player2":
            player_cs_left += int(player_second_cs_right)
    elif selected_hand == "right" and target_hand == "right":
        if now_turn == "player1":
            player_second_cs_right += int(player_cs_right)
        elif now_turn == "player2":
            player_cs_right += int(player_second_cs_right)
    ...
    os.system('cls')
    print("당신의 손가락:", player_cs_left, player_cs_right, "컴퓨터의 손가락:", player_second_cs_left, player_second_cs_right)
    if now_turn == "player1":
        player_turn = 2
    elif now_turn == "player2":
        player_turn = 1
...
def separate(now_turn):
    ...
    if now_turn == "player1":
        ...
        if both == (player_cs_left + player_cs_right):
            ...
            os.system('cls')
            print("당신의 손가락:", player_cs_left, player_cs_right, "컴퓨터의 손가락:", player_second_cs_left,
                  player_second_cs_right)
        else:
            print("[올바른 입력이 아닙니다. 다시 입력해주세요]\n")
            separate("player1")
    elif now_turn == "player2":
        sel_hand = random.choice(["left", "right"])
        selected = random.randint(1, (player_second_cs_right + player_second_cs_left))
        for i in range(0, player_second_cs_right + player_second_cs_left):
            if (player_second_cs_right + player_second_cs_left) - selected > 5:
                selected -= 1
            else:
                if sel_hand == "left":
                    r = selected
                    l = (player_second_cs_right + player_second_cs_left) - selected
                elif sel_hand == "right":
                    r = selected
                    l = (player_second_cs_right + player_second_cs_left) - selected
                if r == player_second_cs_right and l == player_second_cs_left:
                    separate("player2")
                else:
                    player_second_cs_right = r
                    player_second_cs_left = l
                break
        os.system('cls')
        print("당신의 손가락:", player_cs_left, player_cs_right, "컴퓨터의 손가락:", player_second_cs_left,
              player_second_cs_right)
...
def start():
    while True:
        ...
        if gameOver:
            ...
            if who_win == "player1":
                print("당신의 승리!")
            elif who_win == "player2":
                print("컴퓨터의 승리...")
            input("[아무키나 눌러 종료합니다]")
            break

os.system('cls')
print("명령 프롬프트에서 실행해 주세요, 다른 터미널에서 실행 시 예기치 않은 문제가 생길 수 있습니다.")
print("당신의 손가락:", player_cs_left, player_cs_right, "컴퓨터의 손가락:", player_second_cs_left,
              player_second_cs_right)
start()
